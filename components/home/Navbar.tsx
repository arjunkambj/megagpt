...
export default function NavMain(props: NavbarProps) {
  ...
  return (
    ...
    >
      <NavbarContent
        className="gap-4 md:gap-12 rounded-full border border-white/10 bg-gradient-to-b from-white/5 to-white/2 backdrop-blur-md px-6 py-2 shadow-xl"
        justify="center"
      >
        {/* Toggle */}
        ...
        <div className="flex items-center gap-6">
          {navItems.map((item, index) => (
            ...
            >
              <Link
                className="text-white/80 mx-2 hover:text-white hover:scale-105 transition-all duration-300 text-sm font-medium"
                href={item.href}
                onClick={handleNavClick}
              >
                {item.name}
        ...
        <Button
          as={Link}
          className=" rounded-full px-6 py-2 text-md"
          color="primary"
          href="/login"
        >
          Get started
      ...
      {/* Menu */}
      <NavbarMenu
        className="top-[calc(var(--navbar-height)/2)] mx-auto mt-16 max-h-[40vh] max-w-[80vw] rounded-large border border-white/10 bg-gradient-to-b from-white/10 to-white/5 backdrop-blur-md py-6 shadow-xl"
        motionProps={{
